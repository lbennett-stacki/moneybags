...
pub fn build_rpc_client_states() -> HashMap<String, RpcClientState> {
    ...
    let states = if IS_MAINNET {
        vec![
            RpcClientState::new(dotenv!("PUBLIC_MAINNET_RPC_URL"), 10),
            // QUICKNODE_MAINNET_RPC_URL, -- Not currently supported
            RpcClientState::new(dotenv!("HELIUS_MAINNET_RPC_URL"), 10),
            RpcClientState::new(dotenv!("ALCHEMY_MAINNET_RPC_URL"), 10),
        ]
    } else {
        vec![
