...
use crawl_status::table::{CrawlStatus, CrawlStatusRow};
use db::client::db_health_check;
use db::init::init_db;
use pump_fun::program::signatures::{get_pump_fun_program_signatures, TransactionSignature};
use pump_fun::tokens::{get_pump_fun_tokens_from_transactions, PumpFunToken};
use pump_fun::trades::trades::get_token_pump_fun_trades;
use raydium::table::PoolAddress;
...
use token::accounts::get_token_accounts;
...
use trades::store::store_trades;
use trades::table::TradeRow;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("{:?}", db_health_check(&db));

    let (pump_fun_program_signatures_tx, pump_fun_signatures_rx) =
    ...
    let (raydium_pools_tx, raydium_pools_rx) = crossbeam::channel::unbounded::<PoolAddress>();
    let (crawl_status_tx, crawl_status_rx) = crossbeam::channel::unbounded::<CrawlStatusRow>();
    ...
    if targetted_mint_address.is_none() && target_transaction_signature.is_none() {
        let handle = get_pump_fun_program_signatures(
    ...
    if target_transaction_signature.is_some() {
        ...
        // TODO: only if token not already exists
        pump_fun_tokens_tx
            .send((
        ...
        let handle = get_pump_fun_tokens_from_transactions(
            &pump_fun_tokens_tx,
            &pump_fun_trades_tx,
    ...
    let tee_handles = thread::spawn(move || {
        for token in pump_fun_tokens_rx {
            let _ = pump_fun_tokens_tx_token_account_tee.send(token).unwrap();
            let _ = pump_fun_tokens_tx_mint_signatures_tee.send(token).unwrap();
        }
    });
    handles.push(tee_handles);

    let token_accounts_handles = get_token_accounts(
        &token_accounts_tx,
    ...
    // TODO: tee pump fun tokens into get curve signatures

    // TODO: tee pump fun tokens into get raydium pool signatures
    ...
    if let Some(target_transaction_signature) = target_transaction_signature {
        ...
        crawl_status_tx
            .send(CrawlStatusRow {
                ...
                slot: 0,
                is_first_account_signature: true,
                status: CrawlStatus::Pending,
            })
            .unwrap();
    }

    let token_trades_handles = get_token_pump_fun_trades(
        &pump_fun_trades_tx,
