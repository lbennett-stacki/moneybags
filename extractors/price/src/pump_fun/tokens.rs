use crate::{
    cpi::cpi::CpiLog, instructions::{discriminators::build_instruction_discriminators, instruction::Instruction}, pump_fun::instructions::PumpFunInstruction, rpc::{clients::get_rpc_nodes_count, pool::RpcPoolManager}, trades::table::{TradeDirection, TradeRow}, transactions::{config::TRANSACTION_CONFIG, parse::parse_transaction_with_logs, status::is_failed_transaction}, utils::log::log_time
};
    ...
    for thread_index in 0..CONCURRENCY {
        ...
        let handle = thread::spawn(move || {
            while let Ok(pump_fun_signature) = pump_fun_signatures_rx.recv() {
                ...
                let tx = rpc_pool_manager.execute(|client| {
                    client.get_transaction_with_config(&sig, TRANSACTION_CONFIG)
                ...
                match tx {
                    ...
                    Ok(tx) => {
                        ...
                        let (account_keys, raw_message) = if let EncodedTransaction::Json(tx_json) = &tx.transaction.transaction {
                        ...
                        let instructions_with_logs = parse_transaction_with_logs(&tx.transaction.meta, &known_discriminators, &account_keys, &raw_message, &pump_fun_program_address);
                    
                        for (instruction_index,instruction_with_logs) in instructions_with_logs.iter().enumerate() {
                            let coin = match instruction_with_logs.instruction {
                                Some(Instruction::PumpFun(_, PumpFunInstruction::Create((_, coin)))) => coin,
                                Some(Instruction::PumpFun(_, PumpFunInstruction::Buy((_, coin)))) => coin,
                                Some(Instruction::PumpFun(_, PumpFunInstruction::Sell((_, coin)))) => coin,
                                _ => continue,
                            };
