...
pub fn parse_single_pump_fun_instruction(
    ...
    let instruction = match found_discriminator {
        ...
        Some(PumpFunInstructionDiscriminator::Create) => {
            ...
            let args = CreateInstructionArgs::try_from_slice(args_data).unwrap();
        ...
        Some(PumpFunInstructionDiscriminator::Buy) => {
            let args_data = &data[8..].get(..16)?;
            let args = BuyInstructionArgs::try_from_slice(args_data).unwrap();
        ...
        Some(PumpFunInstructionDiscriminator::Sell) => {
            let args_data = &data[8..].get(..16)?;
            let args = SellInstructionArgs::try_from_slice(args_data).unwrap();
