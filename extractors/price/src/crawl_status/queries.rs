...
pub fn has_pending_signature(
    client: &Client,
    signature: &str,
) -> Result<bool, CrawlStatusQueryError> {
    has_signature_status(client, signature, vec!["pending"])
}

pub fn has_seen_signature(client: &Client, signature: &str) -> Result<bool, CrawlStatusQueryError> {
    has_signature_status(client, signature, vec!["succeeded", "failed", "pending"])
}
